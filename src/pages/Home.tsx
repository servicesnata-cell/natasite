...
const CounterComponent = ({ targetValue, label }: { targetValue: number; label: string }) => {
  ...
  return (
    <div ref={sectionRef} className="text-center">
      <div className="text-5xl md:text-6xl font-bold text-white mb-2">{displayValue}+</div>
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          <div className="relative max-w-4xl mx-auto">
            <div
              className="absolute left-1/2 transform -translate-x-1/2 h-full w-1 bg-gradient-to-b from-cyan-400/40 via-purple-400/40 to-emerald-400/40 border-l-2 border-dashed border-white/20"
            ...
            <div className="space-y-20 relative py-8">
              ...
              {['Discover','Plan','Build','Deliver'].map((step, i) => {
                ...
                return (
                  <div key={i} ref={ref} className="flex items-center gap-8">
                    {isLeft ? (
                      <>
                        <div className="w-full md:w-5/12 text-right">
                          <div className={`bg-white/8 border border-white/20 rounded-lg p-6 hover:bg-white/12 transition-all duration-300 hover:shadow-lg ${isVisible ? 'step-visible left-align' : 'opacity-0'}`}>
                            <div className="flex items-center justify-end gap-3 mb-3">
                              <div className="text-right"><h4 className={`text-xl font-bold ${c.text}`}>{step}</h4></div>
                              <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${c.bg} border border-white/50 flex items-center justify-center font-bold text-white shadow-lg shadow-white/30`}></div>
                            </div>
                            <p className="text-gray-300 text-sm">{descriptions[i]}</p>
                          </div>
                        </div>
                        <div className="hidden md:flex w-2/12 justify-center">
                          <div className={`w-4 h-4 rounded-full ${c.dot} ${isVisible ? 'dot-visible' : 'opacity-0'}`}></div>
                        </div>
                      ...
                      <>
                        ...
                        <div className="w-full md:w-5/12">
                          <div className={`bg-white/8 border border-white/20 rounded-lg p-6 hover:bg-white/12 transition-all duration-300 hover:shadow-lg ${isVisible ? 'step-visible right-align' : 'opacity-0'}`}>
                            <div className="flex items-center gap-3 mb-3">
                              <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${c.bg} border border-white/50 flex items-center justify-center font-bold text-white shadow-lg shadow-white/30`}></div>
                              <div><h4 className={`text-xl font-bold ${c.text}`}>{step}</h4></div>
                            </div>
                            <p className="text-gray-300 text-sm">{descriptions[i]}</p>
                          </div>
                        </div>
